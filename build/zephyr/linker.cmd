...
/DISCARD/ :
{
 ...
 KEEP(*(.gnu.attributes))
    ...
    BACKUP_SRAM 947912704 (NOLOAD) : { __BACKUP_SRAM_start = .; KEEP(*(BACKUP_SRAM)) KEEP(*(BACKUP_SRAM.*)) __BACKUP_SRAM_end = .; } > BACKUP_SRAM __BACKUP_SRAM_size = __BACKUP_SRAM_end - __BACKUP_SRAM_start; __BACKUP_SRAM_load_start = LOADADDR(BACKUP_SRAM); QSPI 2415919104 (NOLOAD) : { __QSPI_start = .; KEEP(*(QSPI)) KEEP(*(QSPI.*)) __QSPI_end = .; } > QSPI __QSPI_size = __QSPI_end - __QSPI_start; __QSPI_load_start = LOADADDR(QSPI); SRAM1 805306368 (NOLOAD) : { __SRAM1_start = .; KEEP(*(SRAM1)) KEEP(*(SRAM1.*)) __SRAM1_end = .; } > SRAM1 __SRAM1_size = __SRAM1_end - __SRAM1_start; __SRAM1_load_start = LOADADDR(SRAM1); SRAM2 805322752 (NOLOAD) : { __SRAM2_start = .; KEEP(*(SRAM2)) KEEP(*(SRAM2.*)) __SRAM2_end = .; } > SRAM2 __SRAM2_size = __SRAM2_end - __SRAM2_start; __SRAM2_load_start = LOADADDR(SRAM2); SRAM4 939524096 (NOLOAD) : { __SRAM4_start = .; KEEP(*(SRAM4)) KEEP(*(SRAM4.*)) __SRAM4_end = .; } > SRAM4 __SRAM4_size = __SRAM4_end - __SRAM4_start; __SRAM4_load_start = LOADADDR(SRAM4); DTCM 536870912 (NOLOAD) : { __DTCM_start = .; KEEP(*(DTCM)) KEEP(*(DTCM.*)) __DTCM_end = .; } > DTCM __DTCM_size = __DTCM_end - __DTCM_start; __DTCM_load_start = LOADADDR(DTCM); ITCM 0 (NOLOAD) : { __ITCM_start = .; KEEP(*(ITCM)) KEEP(*(ITCM.*)) __ITCM_end = .; } > ITCM __ITCM_size = __ITCM_end - __ITCM_start; __ITCM_load_start = LOADADDR(ITCM);
.last_section :
{
  LONG(0xE015E015)
} > FLASH
